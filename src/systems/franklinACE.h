...
//
// You need to include the following headers before including franklinACE.h:
//
...
// - chips/mem.h
// - chips/clk.h
// - devices/franklinACE_lc.h
// - devices/disk2_fdd.h
// - devices/disk2_fdc.h
// - devices/franklinACE_fdc_rom.h
...
// Bump snapshot version when franklinACE_t memory layout changes
...
// clang-format off
static uint32_t __not_in_flash() franklinACE_palette[PALETTE_SIZE] = {
...
// Config parameters for franklinACE_init()
typedef struct {
...
typedef struct {
    ...
    franklinACE_lc_t lc;  // Language card
...
void franklinACE_init(franklinACE_t *sys, const franklinACE_desc_t *desc);
// Discard Apple2 instance
void franklinACE_discard(franklinACE_t *sys);
// Reset a Apple2 instance
void franklinACE_reset(franklinACE_t *sys);

void franklinACE_tick(franklinACE_t *sys);

// Tick Apple2 instance for a given number of microseconds, return number of executed ticks
uint32_t franklinACE_exec(franklinACE_t *sys, uint32_t micro_seconds);
// Take a snapshot, patches pointers to zero or offsets, returns snapshot version
uint32_t franklinACE_save_snapshot(franklinACE_t *sys, franklinACE_t *dst);
// Load a snapshot, returns false if snapshot version doesn't match
bool franklinACE_load_snapshot(franklinACE_t *sys, uint32_t version, franklinACE_t *src);

void franklinACE_screen_update(franklinACE_t *sys);
...
static void _franklinACE_init_memorymap(franklinACE_t *sys);

// clang-format off
static uint8_t __not_in_flash() _franklinACE_artifact_color_lut[1<<7] = {
...
static inline uint32_t _franklinACE_rotl4b(uint32_t n, uint32_t count) { return (n >> (-count & 3)) & 0xF; }

static uint16_t __not_in_flash() _franklinACE_double_7_bits_lut[128];

static void _franklinACE_init_double_7_bits_lut() {
    for (uint8_t bits = 0; bits < 128; ++bits) {
        ...
        _franklinACE_double_7_bits_lut[bits] = result;
    }
}

static inline uint16_t _franklinACE_double_7_bits(uint8_t bits) { return _franklinACE_double_7_bits_lut[bits]; }

static inline uint8_t _franklinACE_reverse_7_bits(uint8_t bits) {
    uint8_t result = 0;
    for (int i = 0; i < 7; i++) {
        result <<= 1;
        result |= bits & 1;
...
void franklinACE_init(franklinACE_t *sys, const franklinACE_desc_t *desc) {
    ...
    _franklinACE_init_double_7_bits_lut();

    memset(sys, 0, sizeof(franklinACE_t));
    sys->valid = true;
    ...
    _franklinACE_init_memorymap(sys);

    franklinACE_lc_init(&sys->lc, &(franklinACE_lc_desc_t){&sys->mem, sys->rom});
    ...
    if (desc->fdc_enabled) {
        ...
        //if (CHIPS_ARRAY_SIZE(franklinACE_nib_images) > 0) {
        //    disk2_fdd_insert_disk(&sys->fdc.fdd[0], franklinACE_nib_images[0]);
        //}
    }
...
static void _franklinACE_mem_c000_c0ff_rw(franklinACE_t *sys, uint16_t addr, bool rw) {
    switch (addr & 0xFF) {
        case 0x00:
            if (rw) {
        ...
        default:
            if ((addr >= 0xC070) && (addr <= 0xC07F)) {
                ...
                franklinACE_lc_control(&sys->lc, addr & 0xF, rw);
                if (rw) {
...
static void _franklinACE_mem_rw(franklinACE_t *sys, uint16_t addr, bool rw) {
    if ((addr >= 0xC000) && (addr <= 0xC0FF)) {
        // Apple II I/O Page
        _franklinACE_mem_c000_c0ff_rw(sys, addr, rw);
...
void franklinACE_tick(franklinACE_t *sys) {
    ...
    _franklinACE_mem_rw(sys, sys->cpu.addr, sys->cpu.rw);
...
static void _franklinACE_render_line_monochrome(uint8_t *out, uint16_t *in, int start_col, int stop_col) {
...
static void _franklinACE_render_line_color(uint8_t *out, uint16_t *in, int start_col, int stop_col) {
    ...
    for (int col = start_col; col < stop_col; col++) {
        ...
        for (int b = 0; b < 7; b++) {
            uint8_t c1 = _franklinACE_rotl4b(_franklinACE_artifact_color_lut[w & 0x7F], col * 14 + b * 2);
            w >>= 1;
            uint8_t c2 = _franklinACE_rotl4b(_franklinACE_artifact_color_lut[w & 0x7F], col * 14 + b * 2 + 1);
            w >>= 1;
...
static uint8_t _franklinACE_get_text_character(franklinACE_t *sys, uint8_t code, uint16_t row) {
    ...
    bits = bits & 0x7F;
    bits ^= invert_mask;
    return _franklinACE_reverse_7_bits(bits);
}

static uint8_t *_franklinACE_get_fb_addr(franklinACE_t *sys, uint16_t row) { return &sys->fb[row * (APPLE2_SCREEN_WIDTH / 2)]; }

static void _franklinACE_lores_update(franklinACE_t *sys, uint16_t begin_row, uint16_t end_row) {
...
#define NIBBLE(byte) (((byte) >> (row & 4)) & 0x0F)
        uint8_t *p = _franklinACE_get_fb_addr(sys, row);
...
#undef NIBBLE
        ...
        for (int y = 1; y < 4; y++) {
            memcpy(_franklinACE_get_fb_addr(sys, row + y), _franklinACE_get_fb_addr(sys, row), 40 * 7);
        }
    }
...
static void _franklinACE_text_update(franklinACE_t *sys, uint16_t begin_row, uint16_t end_row) {
    if ((!sys->page2 && !sys->text_page1_dirty) || (sys->page2 && !sys->text_page2_dirty)) {
        return;
    }
    ...
    for (int row = start_row; row < stop_row; row++) {
        ...
        for (int col = 0; col < 40; col++) {
            words[col] = _franklinACE_double_7_bits(_franklinACE_get_text_character(sys, vram_row[col], row & 7));
        }

        _franklinACE_render_line_monochrome(_franklinACE_get_fb_addr(sys, row), words, 0, 40);
    }
...
static void _franklinACE_hgr_update(franklinACE_t *sys, uint16_t begin_row, uint16_t end_row) {
    ...
    for (int row = begin_row; row <= end_row; row++) {
        ...
        for (int col = 0; col < 40; col++) {
            uint16_t w = _franklinACE_double_7_bits(vram_row[col] & 0x7F);
        ...
        _franklinACE_render_line_color(_franklinACE_get_fb_addr(sys, row), words, 0, 40);
    }
...
void franklinACE_screen_update(franklinACE_t *sys) {
    ...
    if (!sys->text) {
        ...
        if (sys->hires) {
            _franklinACE_hgr_update(sys, 0, text_start_row - 1);
        } else {
            _franklinACE_lores_update(sys, 0, text_start_row - 1);
        }
    }

    if (text_start_row < 192) {
        _franklinACE_text_update(sys, text_start_row, 191);
    }
}
