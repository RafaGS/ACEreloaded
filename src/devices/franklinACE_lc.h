...
// Config parameters for franklinACE_lc_init()
typedef struct {
    mem_t* sys_mem;
    uint8_t* sys_rom;
} franklinACE_lc_desc_t;
...
void franklinACE_lc_init(franklinACE_lc_t* dev, const franklinACE_lc_desc_t* desc);

// Discard the language card
void franklinACE_lc_discard(franklinACE_lc_t* dev);

// Reset the language card
void franklinACE_lc_reset(franklinACE_lc_t* dev);

void franklinACE_lc_control(franklinACE_lc_t* dev, uint8_t offset, bool rw);

// Prepare a new language card snapshot for saving
void franklinACE_lc_snapshot_onsave(franklinACE_lc_t* snapshot);

// Fix up the language card snapshot after loading
void franklinACE_lc_snapshot_onload(franklinACE_lc_t* snapshot, franklinACE_lc_t* dev);
...
void franklinACE_lc_init(franklinACE_lc_t* dev, const franklinACE_lc_desc_t* desc) {
    ...
    memset(dev, 0, sizeof(franklinACE_lc_t));
...
void franklinACE_lc_reset(franklinACE_lc_t* dev) { CHIPS_ASSERT(dev && dev->valid); }
...
void franklinACE_lc_snapshot_onsave(franklinACE_lc_t* snapshot) { CHIPS_ASSERT(snapshot); }

void franklinACE_lc_snapshot_onload(franklinACE_lc_t* snapshot, franklinACE_lc_t* dev) { CHIPS_ASSERT(snapshot && dev); }
